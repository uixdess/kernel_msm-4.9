...
static int qpnp_haptics_parse_buffer_dt(struct hap_chip *chip)
{
	...
	if (!rc) {
		...
		if (!prop) {
				dev_info(&chip->pdev->dev, "effect array not found");
			} else if (temp != HAP_WAVE_SAMP_LEN * chip->effect_max) {
				dev_err(&chip->pdev->dev, "Invalid len of effect array \n");
				chip->effect_max = 0;
				return -EINVAL;
			} else {
				chip->effect_array = (u8 (*)[HAP_WAVE_SAMP_LEN])kmalloc(HAP_WAVE_SAMP_LEN * chip->effect_max, GFP_KERNEL);
				memcpy(chip->effect_array, prop->value,
						HAP_WAVE_SAMP_LEN *  chip->effect_max);
				for (temp = 0; temp < chip->effect_max; temp++) {
					pr_info("effect_array:%u: %u,%u,%u,%u,%u,%u,%u,%u\n",
							temp, chip->effect_array[temp][0], chip->effect_array[temp][1],
							chip->effect_array[temp][2], chip->effect_array[temp][3],
							chip->effect_array[temp][4], chip->effect_array[temp][5],
							chip->effect_array[temp][6], chip->effect_array[temp][7]);
				}
			}
	}
	chip->wave_rep_cnt = WF_REPEAT_MIN;
